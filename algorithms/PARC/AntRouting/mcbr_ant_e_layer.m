function status = mcbr_ant_e_layer(N, S)
...
case 'Packet_Received'
    ...
    if (msgID ~= -inf)
        ...
        %record the potentials vector, which store potential for each
        %neghbor node.
    ...
    if (msgID == -1) %forward ant
        if(DESTINATIONS(ID)) %arriving destination
            ...
            antBackward.cost = 0;
            status = mcbr_ant_e_layer(N, make_event(t, 'Send_Packet', ID, antBackward));
        else
            status = mcbr_ant_e_layer(N, make_event(t, 'Send_Packet', ID, data.data));
        end
    end
...
case 'Clock_Tick'
    ...
    if (strcmp(type, 'ant_start'))
        if (isempty(probability{ID}))  %see equation 4 in paper
            ...
            %initlize the probability distribution, which is different with
            %basic ant routing. According to equation 4 in ant04.
            values = exp((cost - memory.potentials)*probGain);       
            probability{ID} = values/sum(values);
        end
        if(SOURCES(ID))
            antForward.msgID = -1;
            status = mcbr_ant_e_layer(N, make_event(t+4000, 'Send_Packet', ID, antForward));             
        end
