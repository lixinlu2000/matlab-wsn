function status = converge_inwardN_layer(N, S)
...
case 'Init_Application'
    ...
    if (ix==1)
        ...
        K = K/10; %inward has small delay constants comparing to outward for the same success rate
        [topology, mote_IDs]=prowler('GetTopologyInfo');
        totalN=length(mote_IDs);
    end  
...
case 'Send_Packet'
    ...
    if (msgID>=0) 
        ...
        det = totalN-h*(h+1)*ns/2;
        if (det<0) det = 0; end
        data.delayTime = K*(det + h*ns*rand);
...
%         end
      ...
      if (~duplicated && ((ATTRIBUTES{ID}.hopcount <= data.data.hopcount) && (msgID >= 0)||...
           ...
           PrintMessage('f');
           status = converge_inwardN_layer(N, make_event(t, 'Send_Packet', ID, data.data)); 
      end
    end
