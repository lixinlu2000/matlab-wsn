function status = converge_control_layer(N, S)
...
%         if (~forward) %add  delay to sending
            ...
            if (dim==2)
              data.delayTime = K*((h-1)/2+rand)*h*ns;
            elseif (dim==1)
              data.delayTime = K*(h-1+rand);
            end
            %data.delayTime = K*((1+ATTRIBUTES{ID}.hopcount)/2-1+rand/4)*ATTRIBUTES{ID}.hopcount*ns;
%         else
%             data.delayTime = ns*rand*h*K; %random delay
%         end
...
%         end
      ...
      if (msgID<0) %update only for backward initialization
      ...
      if (~duplicated && ((ATTRIBUTES{ID}.hopcount <= data.data.hopcount) && (msgID >= 0)||...
           ...
           PrintMessage('f');
           status = converge_control_layer(N, make_event(t, 'Send_Packet', ID, data.data)); 
      end
    end
