...
% This implementation is disgned for EEABR ant routing protocol discribed in EEABR.
% 
% Written by Xinlu Li, xinlu.li@mydit.ie 29/09/2017
...
case 'Init_Application'  % Initilize Application
    ...
    pheromone{ID} = [];
    agent_id = 1;
    statistics = struct('source',0,'destination',0,'agent_id',agent_id,'hops',0,'avgValue',0,'minValue',0,'maxValue',0);
    staitstics_idx = 1;
...
case 'Packet_Received'
    ...
    if (msgID == -1) % receive forward ant
        if(DESTINATIONS(ID)) %arriving destination
            ...
            statistics(staitstics_idx).source = rdata.source;
            statistics(staitstics_idx).destination = ID;
            statistics(staitstics_idx).hops = length(rdata.list);
            statistics(staitstics_idx).agent_id = rdata.agent_id;
            statistics(staitstics_idx).maxValue = maxValue;
            statistics(staitstics_idx).minValue = minValue;
            statistics(staitstics_idx).avgValue = avgValue;
            
            staitstics_idx = staitstics_idx + 1;
...
%             phermone_increment = 1/(initPower - (avgValue - 1/minValue));
    ...
    if (msgID == -2) %backward ant
        ...
        pheromone{ID} =  Set_New_PH(pheromone{ID},nID,tmp_pheromone,evaporation);
...
% writen by xinlu 
% update pheromone trail according to equation 6 in EEABR
