...
case 'Init_Application'  % Initilize Application
    ...
    heuristic{ID} = [];
    agent_id = 1;
    statistics{ID} = struct('source',ID,'destination',0,'agent_id',agent_id,'hops',0,'avgValue',0,'minValue',0,'maxValue',0,'ph_increment',0);
    statistics_idx = 1;
...
%             if(ismember(find(DESTINATIONS),NEIGHBORS{ID}))
...
case 'Packet_Received'
    ...
    if (msgID == -1) % receive forward ant
        if(DESTINATIONS(ID)) %arriving destination
            ...
            statistics{rdata.source}.source = rdata.source;
            statistics{rdata.source}.destination = ID;
            statistics{rdata.source}.hops = length(rdata.list);
            statistics{rdata.source}.agent_id = rdata.agent_id;
            statistics{rdata.source}.maxValue = maxValue;
            statistics{rdata.source}.minValue = minValue;
            statistics{rdata.source}.avgValue = avgValue;
            
%             statistics(statistics_idx).source = rdata.source;
%             statistics(statistics_idx).destination = ID;
%             statistics(statistics_idx).hops = length(rdata.list);
%             statistics(statistics_idx).agent_id = rdata.agent_id;
%             statistics(statistics_idx).maxValue = maxValue;
%             statistics(statistics_idx).minValue = minValue;
%             statistics(statistics_idx).avgValue = avgValue;
...
%             phermone_increment = 1/(initPower - (avgValue - 1/minValue));
            phermone_increment = minValue/exp((initPower-avgValue)*fant_length);
            
%             phermone_increment = exp(phermone_increment); %avoid pheromone increment too low
            
            antBackward.ph_increment = phermone_increment;
%             statistics(statistics_idx).ph_increment = phermone_increment;
            statistics{rdata.source}.ph_increment = phermone_increment;
            
            statistics_idx = statistics_idx + 1;
    ...
    if (msgID == -2) %backward ant
        ...
        tmp_list = 1:rdata.path_length;
        tmp_sum = sum(tmp_list);
%         tmp_pheromone = statistics(statistics_idx - 1).ph_increment;
        tmp_pheromone = statistics{rdata.from}.ph_increment;
        
        ph_pheromone = tmp_pheromone * (length(rdata.list) + 1) / tmp_sum;
        
%         tmp_pheromone = rdata.ph_increment * (length(rdata.list) + 1) / tmp_sum;
        pheromone{ID} =  Set_New_PH(pheromone{ID},nID,ph_pheromone,evaporation);
        
        %updata heuristic value
        avg_Value = statistics(statistics_idx - 1).avgValue;
...
case 'Clock_Tick'
    ...
    if (strcmp(type, 'ant_start'))
        ...
        if(SOURCES(ID))
            antForward.msgID = -1;
            antForward.source = ID;
            antForward.agent_id = agent_id;
            %backDelayMin = 4000
            %backDelayRand = 2000
            %t+backDelayMin+backDelayRand*rand
